...
export class Task implements ITask {
	...
	constructor(task: Partial<ITask>) {
		// Task instance will have the some of the properties of ITask interface
		Object.assign(this, task);

		if (!task.subtasks || task.subtasks.length === 0) delete this.subtasks; // delete if subtask is not available
		else this.subtasks = task.subtasks.map((sub) => new Task(sub)); // recursively
	}
	/////
	...
	stringify = (availableStates: TaskState[], options?: StringifyArgs) => {
		...
		// const taskState = availableStates.find((state) => this.state === state.name);
		...
		if (!hideDescription) {
			const parseDescriptionLines = () => {
				...
				// write strike through for each line of desc
