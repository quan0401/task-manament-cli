...
export namespace System {
	...
	/**
	 * Returns as if it's already absolute, otherwise make it absolute
	 */

	export const getAbsolutePath = (filePath: string) => (path.isAbsolute(filePath) ? filePath : path.join(process.cwd(), filePath));

	export const readJSONFile = (relativePath: string): any => {
		let file;
		let datas = {};
		try {
			file = fs.readFileSync(getAbsolutePath(relativePath), { encoding: 'utf-8', flag: 'r' });
		} catch (error) {
